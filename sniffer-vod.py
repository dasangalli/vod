#!/usr/bin/env python3
"""
sniffer-vod.py — HLS Manifest Sniffer per Nginx Proxy Live (Cloudflare Worker Edition)
"""
...
class ManifestSniffer:
    """Intercetta la prima richiesta di manifest media nella pagina."""
...
def generate_stream_conf(stream_id: str, target_url: str, m3u8_url: str, req_headers: dict, worker_url: str) -> str:
...
location ~ ^/live/{stream_id}/segment/(.+)$ {{
    resolver 8.8.8.8 valid=30s;
    set $worker_url "{worker_url}";
    set $segment_target "{base_hls_url}$1";
...
def run(playwright: Playwright, url: str, stream_id: str, headless: bool, wait_seconds: float, conf_dir: str, out_dir: str, worker_url: str) -> None:
    ...
    if not first:
        print("[-] Nessun manifest .m3u8 rilevato.", file=sys.stderr)
        browser.close()
        sys.exit(1)
    ...
    # 1. Salva la configurazione Nginx in conf_dir
    conf_content = generate_stream_conf(stream_id, url, target_url, clean_headers, worker_url)
    ...
    # 2. Salva index.html e streams.json in out_dir
    index_path = out_folder / "index.html"
    index_path.write_text(INDEX_HTML_TEMPLATE, encoding="utf-8")
    print(f"[✔] Player Web salvato in: {index_path.resolve()}", file=sys.stderr)
...
def main() -> None:
    ...
    parser.add_argument("--conf-dir", type=str, default=".", help="Cartella configurazioni Nginx (default: cartella corrente)")
    parser.add_argument("--out-dir", type=str, default=".", help="Cartella Web Player (default: cartella corrente)")
    parser.add_argument("--worker-url", type=str, required=True, help="URL del Cloudflare Worker di proxy")
    parser.add_argument("--headed", action="store_true", help="Esegui browser visibile")
    parser.add_argument("--wait", type=float, default=6.0, help="Secondi di attesa (default: 6)")
